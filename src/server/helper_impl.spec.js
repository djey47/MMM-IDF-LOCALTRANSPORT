...
const mockGetTransilienStopMonitoringUrl = jest.fn();

jest.mock('axios', () => ({
  get: (url, axiosConfig) => mockAxiosGet(url, axiosConfig),
}));

jest.mock('../support/api/transilien', () => ({
  getTransilienStopMonitoringUrl: (a, sc) => mockGetTransilienStopMonitoringUrl(a, sc),
}));
...
describe('node helper implementation', () => {
  ...
  describe('updateTimetable function', () => {
    beforeEach(() => {
      mockGetTransilienStopMonitoringUrl.mockReset();
    });
    ...
    it('should send UPDATE notifications and invoke getResponse function', () => {
      ...
      NodeHelperImpl.config = {
        ...
        stations: [{
          ...
          line: 'L',
          transilienRefData: {
            stopAreaRef: '87382002',
          },
        }],
      };
      NodeHelperImpl.getResponse = getResponseMock;  
      const transilienStopConfig = NodeHelperImpl.config.stations[4];
      mockGetTransilienStopMonitoringUrl.mockReturnValue('http://transilien-stop-monitoring.api/');
      // when
      ...
      expect(getResponseMock).toHaveBeenCalledWith(
        'http://transilien-stop-monitoring.api/',
        TransilienResponseProcessor.processTransportTransilien,
        'token',
        transilienStopConfig,
      );
      expect(mockGetTransilienStopMonitoringUrl).toHaveBeenCalledWith('http://apiTransilien/', transilienStopConfig);
    });
  });
